...
const ApplicantsTable = () => {
    ...
    const statusHandler = async (status, id) => {
        try {
            const res = await axios.post(`https://career-connect-79k9.onrender.com/api/v1/application/status/${id}/update`, {status}, {
    ...
    return (
        <Table className="bg-slate-100 shadow-xl">
                ...
                {applicants && applicants?.applications?.map((item) => (
                    <motion.tr
                        ...
                        <TableCell className="float-right cursor-pointer">
                            <Popover className="bg-slate-100">
